...
const CalendarSection = ({ userId, onCalendarPreferenceSaved }) => {
  const [officeHours, setOfficeHours] = useState([]);
  const [isSaving, setIsSaving] = useState(false);
  const [message, setMessage] = useState(null);
  const [calendarFormat, setCalendarFormat] = useState(CALENDAR_TYPES.AD);
  ...
  const fetchCalendarPreference = async () => {
    try {
      const value = await invoke("get_calendar_preference", { userId });
      setCalendarFormat(
        value === CALENDAR_TYPES.BS ? CALENDAR_TYPES.BS : CALENDAR_TYPES.AD,
      );
    } catch (err) {
      console.error("Failed to fetch calendar preference:", err);
      setMessage({
        type: "error",
        text: "Failed to load calendar preference.",
      });
    }
  };
  ...
  const saveOfficeHours = async () => {
    ...
    try {
      ...
      await invoke("save_calendar_preference", {
        payload: {
          user_id: userId,
          calendar_preference: calendarFormat,
        },
      });
      if (onCalendarPreferenceSaved) {
        onCalendarPreferenceSaved(calendarFormat);
      }
      setMessage({
        type: "success",
      ...
      setMessage({ type: "error", text: "Failed to save calendar settings." });
    } finally {
      setIsSaving(false);
    }
  };
      ...
      <div className="section-header">
        ...
        <div className="header-controls">
          <div className="format-selector">
            ...
            >
              <option value={CALENDAR_TYPES.BS}>Nepali (B.S.)</option>
              <option value={CALENDAR_TYPES.AD}>English (A.D.)</option>
            </select>

            <div className="tooltip-container">
              <HelpCircle size={18} className="help-icon" />
              <div className="tooltip-content">
                Switch between Nepali Bikram Sambat (B.S.) and English Anno
                Domini (A.D.) calendar types for all date displays.
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="hours-list">
            {officeHours.map((hour) => (
              <div
                ...
                className={`day-settings-row ${hour.is_off_day ? "is-off" : ""}`}
              >
        ...
        <div className="section-footer">
          ...
          >
            {isSaving ? (
              ...
              <>
                <Save size={18} /> Save Calendar Settings
              </>
            )}
          </button>
        </div>
...
const SettingsPage = ({ user, onCalendarPreferenceSaved }) => {
  const [subSection, setSubSection] = useState("profile");

  return (
    <div className="settings-page-wrapper">
      <SettingsSidebar activeSection={subSection} onSelect={setSubSection} />
      ...
      <main className="settings-content-area">
        ...
        {subSection === "calendar" && (
          <CalendarSection
            userId={user?.id}
            onCalendarPreferenceSaved={onCalendarPreferenceSaved}
          />
        )}
      </main>
    </div>
